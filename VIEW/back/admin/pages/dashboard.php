...
try {
    ...
    // Prepare SQL to get the total view counts for each day of the week
    $stmt = $conn->prepare("SELECT view_date, SUM(view_count) AS total_views 
                            FROM views 
    ...
    // Populate the data arrays with the results
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $days[] = $row['view_date'];
        $views[] = $row['total_views'];
    }

    // Add days with 0 views if they are missing from the result set
    $currentDate = strtotime($startOfWeek);
    while ($currentDate <= strtotime($endOfWeek)) {
        $dateStr = date('Y-m-d', $currentDate);
        if (!in_array($dateStr, $days)) {
            $days[] = $dateStr;
            $views[] = 0; // No views recorded for this date
        }
        $currentDate = strtotime("+1 day", $currentDate);
    }

    // Convert data to JSON for use in JavaScript
    $chartData = json_encode(['days' => $days, 'views' => $views]);

    // Get today's date
    $today = date('Y-m-d');

    // Query to get the total number of views for today
    $todayStmt = $conn->prepare("SELECT SUM(view_count) AS total_views_today FROM views WHERE view_date = :view_date");
    $todayStmt->execute([':view_date' => $today]);
    $todayResult = $todayStmt->fetch(PDO::FETCH_ASSOC);

    // Get the total views for today, defaulting to 0 if no views exist
    $totalViewsToday = $todayResult['total_views_today'] ? $todayResult['total_views_today'] : 0;
...
<aside class="sidenav navbar navbar-vertical navbar-expand-xs border-radius-lg fixed-start ms-2 my-2" id="sidenav-main" style="background-color: #2e59d9;">
  ...
  <li class="nav-item">
    <a class="nav-link text-white" href="../../..\back\listeRec.php">
      
      <span class="nav-link-text ms-1">Reclamation</span>
    </a>
  </li>
  <li class="nav-item">
    <a class="nav-link text-white" href="../../..\back\affiche.php">
     
      <span class="nav-link-text ms-1">EVENT</span>
    </a>
  </li>
  <li class="nav-item">
    <a class="nav-link text-white" href="../../..\back\backoffice.php">
      <span class="nav-link-text ms-1">SHOP</span>
    </a>
  </li>
  <li class="nav-item">
    <a class="nav-link text-white" href="../../..\back\dashboardblog.php">
      <span class="nav-link-text ms-1">Blog</span>
    </a>
  </li>
  <li class="nav-item">
    <a class="nav-link text-white" href="../../..\back\createQuiz.php">
  ...
  <h4 class="mb-0"><?php echo number_format($totalViewsToday); ?></h4>
</div>
      ...
      <div class="chart-container">
        <h2 class="chart-title">Weekly Page Views</h2>
        <canvas id="chart-bars" width="800" height="400"></canvas>
    </div>

    <script>
        // Ensure this JSON object is correctly output from PHP
