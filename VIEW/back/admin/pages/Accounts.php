...
if (isset($_POST['update'])) {
    ...
    // Check if a file was uploaded and handle the image upload
    if ($image) {
        ...
        if (!in_array($file_extension, $allowed_extensions)) {
            // If the file extension is not allowed, show an error
            echo "Error: Only image files (jpg, jpeg, png, gif) are allowed.";
            exit();
        }

        // Check file size (e.g., max size of 5MB)
        $max_file_size = 5 * 1024 * 1024; // 5MB
        if ($_FILES['image']['size'] > $max_file_size) {
            echo "Error: The file is too large. Max size is 5MB.";
            exit();
        }

        // Create a unique filename by appending a timestamp to avoid overwriting
        ...
        if (move_uploaded_file($image_tmp_name, $image_folder)) {
            ...
            $update->execute([$name, $email, $user_type, $unique_image_name, $id]);
        } else {
            ...
            <div class="modal-dialog">
                <div class="modal-content" style="border-radius: 10px; overflow: hidden; box-shadow: 0px 4px 10px rgba(0, 0, 0, 0.1);">
                    ...
                    <div class="modal-body" style="padding: 20px;">
                        <form action="Accounts.php" method="POST" enctype="multipart/form-data" style="display: flex; flex-direction: column; gap: 15px;">
                            ...
                            <?php if (!empty($user['image'])): ?>
                                <div style="display: flex; align-items: center;">
                                    <label style="width: 100px; font-weight: bold;">Current Image</label>
                                    <img src="uploaded_img/<?php echo htmlspecialchars($user['image']); ?>" alt="Current Image" style="max-width: 100px; height: auto; border-radius: 5px;">
                                </div>
                            <?php endif; ?>
